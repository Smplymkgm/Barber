...
exports.handler = async function(event) {
  ...
  try {
    ...
    if(event.httpMethod === 'POST'){
      ...
      if(type === 'services'){
        await sql`INSERT INTO services (id, name, name_es, description, price, badge, sort_order)
    ...
    if(event.httpMethod === 'PUT'){
      ...
      if(type === 'services'){
        ...
        if(fields.desc !== undefined) await sql`UPDATE services SET description = ${fields.desc} WHERE id = ${id}`;
        return { statusCode: 200, headers, body: JSON.stringify({ success: true }) };
      }
      if(type === 'coupons'){
